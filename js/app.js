...
const getBlogs = async () => {
  ...
  try {
    ...
    for (let i = 0; i < blogsItems.length; i++) {
      ...
      const altText = blogsItems[i]._embedded["wp:featuredmedia"][0].alt_text;
      const blogTitle = blogsItems[i].title.rendered;
      const categoryId = blogsItems[i].categories[0] || blogsItems[i].categories[1] || blogsItems[i].categories[2];
      ...
      if (categoryName.startsWith("L")) {
        slider.innerHTML += `
          <div class="cards cardSlides">
            <img class="cardsImg" src="${featuredImage}" alt="${altText}">
            <a href="details.html?id=${blogsItems[i].id}&title=${blogTitle}">
              <h5 class="blogTitle">${blogTitle}</h5>
              <div class="smallTags">
                <p>${blogsItems[i].excerpt.rendered}</p>
                <small>${categoryName}</small>
        ...
        trendingPostsElements.innerHTML += `
          <div class="cards trendingPosts">
            <img class="cardsImg" src="${featuredImage}" alt="${altText}">
            <a href="details.html?id=${blogsItems[i].id}&title=${encodeURIComponent(blogTitle)}">
              <h5 class="blogTitle">${blogTitle}</h5>
              <div class="smallTags">
                <p>${blogsItems[i].excerpt.rendered}</p>
                <small>${categoryName}</small>
        ...
        `;
      } else if (categoryName.startsWith("E") && latestFromElements.children.length < 6) {
        latestFromElements.innerHTML += `
          <div class="cards latestFrom">
            <img class="cardsImg" src="${featuredImage}" alt="${altText}">
            <a href="details.html?id=${blogsItems[i].id}&title=${encodeURIComponent(blogTitle)}">
